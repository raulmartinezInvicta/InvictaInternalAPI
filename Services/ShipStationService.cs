...
namespace InvictaInternalAPI.Services
{
    public class ShipStationService : IShipStationService
    {
        ...
        public ShipStationService(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        public async Task<bool> CancelShipStationOrder(CancelOrder cancelOrder)
        {
                ...
                if (getSuccess)
                {
                    ...
                    foreach (SSOrder orderEntry in myDeserializedClass.orders)
                    {
                        ...
                        {   // some missmatch
                            ...
                            {
                                if (retryUpdate2 > 0) System.Threading.Thread.Sleep(2000);
                                HttpContent cCreatePost = new StringContent(createOrderJson2, System.Text.Encoding.UTF8, "application/json");
                                var createPost = await httpClient.PostAsync(uPost2, cCreatePost);
